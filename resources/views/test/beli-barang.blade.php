...
<!-- Mirrored from odindesign-themes.com/emerald-dragon/item-v2.html by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 15 Mar 2019 18:24:45 GMT -->
<head>
	<meta charset="UTF-8">
	...
	<title>ePakan | Beli-barang</title>
</head>
<body>
	...
	<!-- SECTION HEADLINE -->
	<div class="section-headline-wrap" style="height:100px">
		...
		<div class="section">
			...
			<div class="sidebar right">
				...
				<div class="sidebar-item void buttons">
					...
					<a  class="btn btn-light btn-block" style="height:40px;margin-bottom:10px">
						<p align="center" style="color:#000"><span>Rp<spam>{{$produk->harga}}<span>/<span>{{$produk->satuan}}</p>
					</a>
            @if(Auth::guard('pengguna')->check())
						@if($produk->pengguna->id != Auth::guard('pengguna')->user()->id)
							<a href="{{Route('pembayaran',$produk->id)}}" class="btn btn-warning btn-block" style="height:40px;margin-bottom:10px">
								<p align="center" style="color:#ffffff"> <i class="fas fa-money-bill-wave"> </i> Pesan Sekarang</p>
							</a>
						@endif
            @else
							<a href="{{ route('loginOTP') }}" class="btn btn-warning btn-block" style="color:#ffffff" role="button" aria-pressed="true"> <i class="fas fa-money-bill-wave"> </i> pesan sekarang</a>
            @endif

            @if(Auth::guard('pengguna')->check())
						@if($produk->pengguna->id != Auth::guard('pengguna')->user()->id)
							<!-- <a href="{{Route('test12')}}" class="btn btn-success btn-block" style="height:40px">
								<button type=submit align="center" style="color:#ffffff"><i class="fas fa-shopping-cart"> </i> Tambah Kerajang</button>
							</a> -->
							<form method="post" action="{{route('tambah-keranjang')}}">
								@csrf
								<input type="hidden" name="id_produk" value="{{$produk->id}}">
								<input type="hidden" name="jumlah" value="1">
								<input type="hidden" name="id_pengguna" value="{{Auth::guard('pengguna')->user()->id}}">
								<button class="btn btn-success btn-block" type="submit"><i class="fas fa-shopping-cart"> </i> Tambah Kerajang</button>
							</form>
						@endif
                    @else
					<a href="{{ route('loginOTP') }}" class="btn btn-success btn-block" style="color:#ffffff" role="button" aria-pressed="true">  <i class="fas fa-shopping-cart"> </i> Tambah Keranjang</a>
                    @endif
				...
				<div class="sidebar-item author-bio short">
					<h4>{{$produk->pengguna->nama}}</h4>
					<hr class="line-separator">
					<!-- USER AVATAR -->
					<a href="{{ route('profil-produk',['id'=>$produk->pengguna->id, 'kategori'=>'Semua Kategori']) }}" class="user-avatar-wrap medium">
						<figure class="user-avatar medium">
							<img src="{{asset('uploads/file/'. $produk->pengguna->foto)}}" alt="profile-default-image">
						</figure>
					</a>
					<!-- /USER AVATAR -->
					
					<p class="text-oneline">Penjual Pakan<br>{{$produk->pengguna->daerah}}</p>
					<!-- SHARE LINKS -->
					
					<!-- /SHARE LINKS -->
					<div class="clearfix"></div>

					<a href="{{ route('profil-produk',['id'=>$produk->pengguna->id, 'kategori'=>'Semua Kategori']) }}" class="btn btn-success btn-block" style="height:40px">
						<p align="center" style="color:#ffffff"> Lihat Profil Penjual</p>
					</a>
                        ...
                        @if(Auth::guard('pengguna')->check())
							@if($produk->pengguna->id != Auth::guard('pengguna')->user()->id)
								<button type="button" class="btn btn-success btn-block" data-toggle="modal" data-target="#exampleModal">Hubungi Penjual</button>
							@endif
                        @else
						<a href="{{ route('loginOTP') }}" class="btn btn-success btn-block" style="color:#ffffff" role="button" aria-pressed="true">  Hubungi Penjual</a>
                        @endif
                        <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                   <div class="modal-body">
                                        <center>{{$produk->pengguna->no_telp}}</center>
                                    </div>
                                </div>
				...
				<div class="sidebar-item product-info">
					...
					<div class="information-layout v2">
						<div class="information-layout-item">
							<p>Status Barang :{{$produk->status}}</p>
							<p>Minimum Pembelian:{{$produk->minimum}} Item</p>
							<p>Tersedia Stok :{{$produk->stok}} Item</p>
						</div>
			...
			<div class="content left">
				<!-- POST -->
				<article class="post" style="height:auto">
					<!-- POST IMAGE -->
					    <div id="carouselExampleIndicators" class="carousel slide" data-ride="carousel">
                                ...
                                <div class="carousel-inner">
                                    ...
                                    <img src="{{ $produk->foto == null ? asset('images/items/pakan.jpg') : asset('uploads/file/' . $produk->foto) }}" class="d-block w-100" alt="...">
                                    </div>
                                    <div class="carousel-item">
                                    <img src="{{ $produk->foto2 == null ? aseet('images/items/pakan.jpg') : asset('uploads/file/' . $produk->foto2) }}" class="d-block w-100" alt="...">
                                    </div>
                                    <div class="carousel-item">
                                    <img src="{{ $produk->foto2 == null ? asset('images/items/pakan.jpg') : asset('uploads/file/' . $produk->foto3) }}" class="d-block w-100" alt="...">
                                    </div>
                                </div>
						...
						<span class="badge badge-pill badge-success">{{$produk->kategori}}</span>
					<!-- /POST IMAGE SLIDES -->

					<hr class="line-separator">
					<div>
					<h2 style="color:#000"> {{$produk->nama}}<h2></div><hr/>

					<!-- POST CONTENT -->
					<div class="post-content">
						<!-- POST PARAGRAPH -->
						<div class="post-paragraph">
							<h4 class="post-title">Deskripsi Produk</h4>
...
<!-- Mirrored from odindesign-themes.com/emerald-dragon/item-v2.html by HTTrack Website Copier/3.x [XR&CO'2014], Fri, 15 Mar 2019 18:25:05 GMT -->
</html>
