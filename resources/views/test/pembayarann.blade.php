<?php
// var_dump($produk[0]->id_keranjang);
?>
<!DOCTYPE html>
<html lang="en">
	...
	<div class="section-wrap">
		<div class="section">
			...
			@if($id == null)
				...
				<form method="post" action="{{route('bayarKeranjang')}}" enctype="multipart/form-data">
					@csrf
                <!-- CART -->
				...
				@else
					<form method="post" action="{{route('bayarKeranjang')}}" enctype="multipart/form-data">
						@csrf
									<!-- CART -->
					<input type="hidden" value="{{$id}}" name="id_pembeli">
			@endif
				...
				<div class="cart"  style="height:auto">
					...
					<div class="cart-item" style="padding-top:0px">
                        ...
                        <div class="container">
							...
							@if($id == null)		
									<?php
										...
										@foreach($produk as $ulang => $object)
											@if ($ulang === 0)
											@endif
                            ...
                            <div class="row">
								...
								@endforeach
									...
									<?php
										...
										<?php
										$jumlahharga+=$object->harga*$object->jumlah;
										?>		
								<div class="col-md-4">
                                  <b>Foto Produk</b>
                                </div>
							...
							<div class="row">
                                <div class="col-md-4">
                                     <img src="{{ asset('uploads/file/'. $object->foto) }}" height="150" width="150">
                                </div>
                                <div class="col-md-4">
								<span>{{$object->deskripsi}} <br><b>Rp. {{$object->harga}} / {{$object->satuan}}</b></span>
                                </div>
								<input type = "hidden" name="harga[]" value = "{{$object->harga}}">
								<input type = "hidden" name="jumlah[]" value = "{{$object->jumlah}}">
								<input type = "hidden" name="id_produk[]" value = "{{$object->id_produk}}">
								<input type = "hidden" name="pembeli[]" value = "{{$object->pembeli}}">
								<input type = "hidden" name="penjual[]" value = "{{$object->id_penjual}}">
								<input type = "hidden" name="id_keranjang[]" value = "{{$object->id_keranjang}}">
                                <div class="col-md-4">
								<div class="btn-group mr-2" role="group" aria-label="Second group" style="border: 1px #dddddd solid;border-radius: 0.5em;">
								<button onclick="fungsiKurang{{$ulang+1}}()" id="btnKurang{{$ulang+1}}" type="button" class="btn btn-success"><i class="fas fa-minus"></i></button>
                                <input id="jumlah{{$ulang+1}}" style="width:100%;max-width:60px" type="number" value="{{$object->jumlah}}" disabled="">
						...
						<div class="cart-total">
							<p name="x" class="price" id="harga">{{$jumlahharga}}</p><p class="price"><span>Rp.</span></p>
						...
						<!-- <div class="cart-total">
							<p class="price" id="admin">{{$object->harga/100}}</p><p class="price"><span>Rp.</span></p>
